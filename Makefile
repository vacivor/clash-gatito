...
.PHONY: build-release package-linux package-linux-tar package-macos package-macos-zip package-windows-zip ci-install-rust ci-install-linux-deps ci-linux-packages ci-windows-package ci-flatpak-prep
...
package-macos-zip: package-macos
	ditto -c -k --keepParent "$(OUTPUT_DIR)/Clash Gatito.app" "$(OUTPUT_DIR)/$(PACKAGE_NAME)-$(VERSION)-macos-$(DIST_ARCH).app.zip"
